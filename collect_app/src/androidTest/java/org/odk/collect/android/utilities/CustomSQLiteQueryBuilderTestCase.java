...
public class CustomSQLiteQueryBuilderTestCase {
    ...
    @Test
    public void copyTableTableTest() {
        createTestTable();
        insertExampleData(TEST_TABLE_NAME);
        readValues(TEST_TABLE_NAME);
        assertTrue(tableExists(TEST_TABLE_NAME));
        assertFalse(tableExists(TEST_TABLE_NAME_2));
    ...
    private void readValues(String tableName) {
        ...
        if (cursor != null) {
            try {
                while (cursor.moveToNext()) {
                    ...
                    int col1ColumnIndex = cursor.getColumnIndex("col1");
                    int col2ColumnIndex = cursor.getColumnIndex("col2");
                    int col3ColumnIndex = cursor.getColumnIndex("col3");

                    assertEquals(String.valueOf(index), cursor.getString(idColumnIndex));
                    assertEquals("col" + index + "x1Value", cursor.getString(col1ColumnIndex));
                    assertEquals("col" + index + "x2Value", cursor.getString(col2ColumnIndex));
                    assertEquals("col" + index + "x3Value", cursor.getString(col3ColumnIndex));
