...
public class ItemsetDbAdapter {
    ...
    public boolean createTable(String formHash, String pathHash, String[] columns, String relativeCsvFilePath) {
        ...
        cv.put(KEY_PATH, new StoragePathProvider().getFormDbPath(relativeCsvFilePath));
    ...
    public void dropTable(String pathHash, String relativeCsvFilePath) {
        ...
        String[] whereArgs = {
                new StoragePathProvider().getFormDbPath(relativeCsvFilePath)
        };
    ...
    public void delete(String relativeCsvFilePath) {
        ...
        if (c != null) {
            if (c.getCount() == 1) {
                c.moveToFirst();
                String table = getMd5FromString(new StoragePathProvider().getAbsoluteFormFilePath(c.getString(c.getColumnIndex(KEY_PATH))));
                db.execSQL("DROP TABLE IF EXISTS " + DATABASE_TABLE + table);
            }
            c.close();
        }
