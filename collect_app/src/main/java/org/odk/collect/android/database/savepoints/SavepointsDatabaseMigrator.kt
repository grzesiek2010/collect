...
import org.odk.collect.android.database.savepoints.SavepointsDatabaseColumns.INSTANCE_DIR_PATH
    ...
    private fun createSavepointsTableV1(db: SQLiteDatabase) {
        db.execSQL(
            "CREATE TABLE IF NOT EXISTS " + SAVEPOINTS_TABLE_NAME + " (" +
                ...
                FORM_DB_ID + " text not null, " +
                INSTANCE_DB_ID + " text not, " +
                SAVEPOINT_FILE_PATH + " text not null, " +
                INSTANCE_DIR_PATH + " text not null);"
        )
    }
}
