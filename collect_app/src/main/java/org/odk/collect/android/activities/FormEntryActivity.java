...
import org.odk.collect.android.listeners.SwipeManager;
...
public class FormEntryActivity extends CollectAbstractActivity implements AnimationListener,
        FormLoaderListener, AdvanceToNextListener, SwipeManager.OnSwipeListener,
    ...
    public void allowSwiping(boolean doSwipe) {
        swipeManager.setAllowSwiping(doSwipe);
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        nextButton.setOnClickListener(v -> {
            swipeManager.setBeenSwiped(true);
            showNextView();
        });
    ...
    public boolean dispatchTouchEvent(MotionEvent mv) {
        boolean handled = swipeManager.getGestureDetector().onTouchEvent(mv);
        if (!handled) {
    ...
    public void showNextView() {
        ...
        try {
            ...
            if (originalEvent == event && originalEvent == FormEntryController.EVENT_END_OF_FORM) {
                swipeManager.setBeenSwiped(false);
                return;
            }
    ...
    public void showView(View next, AnimationType from) {
        ...
        currentView = next;
        swipeManager.setOdkView(getCurrentViewIfODKView());
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_DPAD_RIGHT:
                if (event.isAltPressed() && !swipeManager.beenSwiped()) {
                    swipeManager.setBeenSwiped(true);
                    showNextView();
                    return true;
                }
