...
public class FormUtils {
        ...
        for (Form form : forms) {
            if (form.getJrFormId().equals(formForChecking.getJrFormId()) && !form.equals(formForChecking)) {
                if (getFormVersion(form) > getFormVersion(formForChecking)
                        || (getFormVersion(form) == getFormVersion(formForChecking) && form.getDate() > formForChecking.getDate())) {
                    return false;
                }
            }
        }
    ...
    private static int getFormVersion(Form form) {
        return form.getJrVersion() != null ? Integer.parseInt(form.getJrVersion()) : -1;
    }
}
