...
class MainMenuViewModel(
    ...
    fun getFormSavedSnackbarType(uri: Uri): FormSavedSnackbarType? {
        val instance = instancesRepository.get(ContentUriHelper.getIdFromUri(uri))
        if (instance != null) {
            return when (instance.status) {
                Instance.STATUS_INCOMPLETE -> FormSavedSnackbarType.SAVED_AS_DRAFT
                Instance.STATUS_COMPLETE -> {
                    val form = formsRepository.getAllByFormIdAndVersion(instance.formId, instance.formVersion).first()
