...
public class DownloadFormListTask extends AsyncTask<Void, String, HashMap<String, FormDetails>> {
            ...
            for (int i = 0; i < elements; ++i) {
                ...
                if (isThisFormAlreadyDownloaded(formId)) {
                    ...
                    if (!isNewerFormVersionAvailable && manifestUrl != null) {
                        List<MediaFile> mediaFiles = downloadMediaFileList(manifestUrl);
                        if (mediaFiles != null) {
                            areNewerMediaFilesAvailable = areNewerMediaFilesAvailable(formId, mediaFiles);
                        }
                    }
                }
    ...
    private boolean areNewerMediaFilesAvailable(String formId, List<MediaFile> mediaFiles) {
        String mediaDirPath = new FormsDao().getFormMediaPathForFormId(formId);
        return !compareLists(mediaFiles, mediaDirPath);
    }

    private boolean compareLists(List<MediaFile> newMediaFiles, String mediaDirPath) {
        boolean result = true;

        if (newMediaFiles != null) {
            List<File> existedMediaFiles = FileUtils.getAllMediaFiles(mediaDirPath);
            if (existedMediaFiles != null) {
                if (existedMediaFiles.size() == newMediaFiles.size()) {
                    for (MediaFile mediaFile : newMediaFiles) {
                        File file = getFileByName(existedMediaFiles, mediaFile.getFilename());
