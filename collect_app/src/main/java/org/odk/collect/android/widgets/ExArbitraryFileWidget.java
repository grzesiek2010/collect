...
public class ExArbitraryFileWidget extends BaseArbitraryFileWidget {
    ...
    protected File saveFileFromUri(Uri uri) {
        File file = FileUtils.createDestinationMediaFile(Collect.getInstance().getFormController().getInstanceFile().getParent(), ContentResolverHelper.getFileExtensionFromUri(getContext(), uri));
        FileUtils.saveAnswerFileFromUri(uri, file, getContext());
        return file;
    }
    ...
    private void onButtonClick() {
        ...
        }, getContext(), getFormEntryPrompt(), activityAvailability);
    }

    private void fireActivity(Intent i) throws ActivityNotFoundException {
        try {
            ((Activity) getContext()).startActivityForResult(i, ApplicationConstants.RequestCodes.EX_ARBITRARY_FILE_CAPTURE);
