...
class BarcodeWidgetTest : QuestionWidgetTest<BarcodeWidget, StringData>() {
    ...
    fun `The button is hidden in read-only mode`() {
        formEntryPrompt = MockFormEntryPromptBuilder(formEntryPrompt)
            ...
            .build()
        createWidget()
        composeRule.onNodeWithClickLabel(composeRule.activity.getString(string.get_barcode)).assertDoesNotExist()
    }
    ...
    fun `Display the 'Replace Barcode' button if answer is present`() {
        formEntryPrompt = MockFormEntryPromptBuilder(formEntryPrompt)
            ...
            .build()
        createWidget()
        composeRule.onNodeWithClickLabel(composeRule.activity.getString(string.replace_barcode)).assertExists()
    }
    ...
    fun `#clearAnswer removes answer and updates button title`() {
        ...
        composeRule.onNodeWithClickLabel(composeRule.activity.getString(string.get_barcode)).assertExists()
    }
    ...
    fun `Long-pressing the button and the answer triggers #onLongClickListener`() {
        createWidget()
        composeRule.onNodeWithClickLabel(composeRule.activity.getString(string.get_barcode)).performSemanticsAction(SemanticsActions.OnLongClick)
        Assertions.assertVisible(withText(string.clear_answer), isDialog())
    }
    ...
    fun `pressing the button with permission not granted does not launch anything`() {
        ...
        composeRule.onNodeWithClickLabel(composeRule.activity.getString(string.get_barcode)).performClick()

        assertThat(Shadows.shadowOf(composeRule.activity).nextStartedActivity, equalTo(null))
    ...
    fun `pressing the button when front camera should be used but it is not available displays a toast`() {
        ...
        assertThat(
            ...
            equalTo(composeRule.activity.getString(string.error_front_camera_unavailable))
        )
    }
    ...
    fun `pressing the button when front camera should be used and it is available launches correct intent`() {
        ...
        assertThat(
            Shadows.shadowOf(composeRule.activity).nextStartedActivity.getBooleanExtra(Appearances.FRONT, false),
            equalTo(true)
        )
    }
